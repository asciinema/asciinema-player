...
(def states
  {:ground {c0-prime? {:action 'execute}
            (events :0x20-0x7F :0xA0-0xFF) {:action 'print}}

   :escape {:on-enter 'clear
   ...
   :csi-param {c0-prime? {:action 'execute}
   ...
   :dcs-param {c0-prime? {:action 'ignore}
               (events :0x20-0x2F) {:action 'collect, :transition :dcs-intermediate}
               (events :0x30-0x39 0x3B) {:action 'param}
   ...
   :dcs-intermediate {c0-prime? {:action 'ignore}
                      (events :0x20-0x2F) {:action 'collect}
                      (events :0x30-0x3F) {:transition :dcs-ignore}
                      (events :0x40-0x7E) {:transition :dcs-passthrough}
                      (events 0x7f) {:action 'ignore}}

   :dcs-passthrough {:on-enter 'hook
                     c0-prime? {:action 'put}
                     (events :0x20-0x7E) {:action 'put}
                     (events 0x7f) {:action 'ignore}
                     :on-exit 'unhook}

   :dcs-ignore {c0-prime? {:action 'ignore}
                (events :0x20-0x7f) {:action 'ignore}}

   :osc-string {:on-enter 'osc-start
   ...
   :sos-pm-apc-string {c0-prime? {:action 'ignore}
                       (events :0x20-0x7F) {:action 'ignore}}})
